...
int currPort;
void writeSocket(char *name, char str[],sem_t* semptrr,int curport){
  char str2[512];
  ...
  saddr.sin_port = htons(curport); /* port number in big-endian */
  ...
  if(curport==9876){
    read(sockfd, threadPortChar, sizeof(threadPortChar));
    currPort=atoi(threadPortChar);
  }
...
int main() {
  ...
  char str[512]="c";
  //scanf(" %[^\n]%*c",&str);
  writeSocket(name,str,semptrr,oldPort);
  while (!sem_wait(semptrr)){
    char str[512];
    printf("String:");
    scanf(" %[^\n]%*c",&str);
    writeSocket(name,str,semptrr,currPort);
  }
  return 0;
}
